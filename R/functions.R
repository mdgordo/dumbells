...
weighted.var <- function(x, w){
  ...
  x <- x[nas]
  w <- w[nas]
  xm <- stats::weighted.mean(x, w)
  wv <- stats::weighted.mean((x-xm)^2)
  return(wv)
}
...
robustses <- function(x, c, setype = "HC1"){
  r <- sqrt(diag(sandwich::vcovPL(x, type = setype, cluster = ~ c)))
  return(r)
}
...
sumstats.wtd <- function(df, wts, labs = NULL, d = 2){
  ...
  means <- apply(df, 2, function(x) stats::weighted.mean(x, wts, na.rm = TRUE))
  sds <- apply(df, 2, function(x) sqrt(weighted.var(x, wts)))
  mins <- apply(df, 2, min, na.rm = TRUE)
  q25s <- apply(df, 2, function(x) stats::quantile(x, .25, na.rm = TRUE))
  q75s <- apply(df, 2, function(x) stats::quantile(x, .75, na.rm = TRUE))
  maxs <- apply(df, 2, max, na.rm = TRUE)
  tab <- data.frame("Statistic" = labs,
                    "N" = formatC(ns, format="d", big.mark=","),
                    "Mean" = formatC(means, format="f", big.mark=",", digits = d),
                    "Std.Dev." = formatC(sds, format="f", big.mark=",", digits = d),
